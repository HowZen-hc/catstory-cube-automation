...
- [ ] AC-1: 珍貴 + 永恆 + 所有屬性 summary 含 `99 力`、`99 敏`、`99 智`、`99 幸`、`77 全`、`12 12 HP`（FR-16, FR-17, FR-18, Signal 5.1, Row 3-ET-AS）
- [ ] AC-2: 珍貴 + 一般 + STR summary 含 `88 力`、`66 全`，不含 `99` / `77`（FR-16, FR-17, Signal 5.2, Row 3-NM-S）
- [ ] AC-3: 絕對附加 + 永恆 + 主屬 / HP 目標 summary 含對應 shorthand（`99 力` / `12 12 HP`）+ 加註 `7 7 全屬（也接受，非全屬職業可於遊戲內轉換裝備職業）`（FR-22, FR-23, FR-24, Signal 5.3, Row A-ET-S / A-ET-HP）
- [ ] AC-4: 絕對附加 + 一般 + `is_glove=True` summary 含 `33 爆`；+ `is_hat=True` summary 含 `-1 -1 冷卻`（FR-24, Signal 5.4, Signal 5.5, Row A-NM-*-G / A-*-H）
- [ ] AC-5: 珍貴 + 永恆 + `is_hat=True` summary 含「-2 冷卻」字樣；珍貴 + 永恆 + `is_glove=True` summary 含「雙爆」但**不含具體 3% 數字**（FR-19, Row 3-ET-S-G / 3-ET-S-H）
- [ ] AC-6: 珍貴 + 主武器 summary 不論選物攻或魔攻皆含 `三物 / 魔`（FR-20, Signal 5.6, Row 3-W）
- [ ] AC-7: 絕對附加 summary 不含 `9 7 雙 S 混搭` 或跨類型組合字樣（FR-22, Signal 5.7）
- [ ] AC-8: 副手 3-line summary 含 `三物 / 三魔` + 「日冕」；2-line summary 保留 v2 結尾 `(副手可於遊戲內進行物魔日冕)`（FR-21, Signal 5.8, Row 3-SW / A-SW）
...
| Development | Pending | 建議順序 — 最後實作，避免中途呈現不一致 |
| Testing | Pending | - |
| Acceptance | Pending | - |
